...
async def test_if_espisod_command_work():
    execute_request_with_episode_reply = await anime_functions.execute_request('episode', 'Elfen Lied', 1, 9)
    async with aiohttp.ClientSession() as session:
        get_episode_reply = await anime_functions.get_episode('episode', 'Elfen Lied', 1, 4)
    assert execute_request_with_episode_reply == get_episode_reply
